...
if not component.isAvailable("stargate") then
	GUI.alert(localization["alert_stargate_required"])
	return
end
...
local function update()
	...
	workspace.connectedToLabel.text = stargateState == "Connected" and localization["connected_to"] .. stargate.remoteAddress() .. ")" or localization["not_connected"]
...
workspace.connectedToLabel = workspace:addChild(GUI.label(x, y, width, 1, 0x555555, localization["not_connected"])):setAlignment(GUI.ALIGNMENT_HORIZONTAL_CENTER, GUI.ALIGNMENT_VERTICAL_TOP); y = y + 2
workspace.connectionButton = workspace:addChild(GUI.framedButton(x, y, width, 3, 0xEEEEEE, 0xEEEEEE, 0xBBBBBB, 0xBBBBBB, localization["connect"])); y = y + 3
-- workspace.connectionButton.animated = false
workspace.connectionButton.onTouch = function()
	if stargate.stargateState() == "Idle" then
		local container = GUI.addBackgroundContainer(workspace, true, true, localization["connect"])
		local input = container.layout:addChild(GUI.input(1, 1, 36, 3, 0xEEEEEE, 0x666666, 0x666666, 0xEEEEEE, 0x262626, contacts.last, localization["type_address_here"]))
...
workspace.irisButton = workspace:addChild(GUI.framedButton(x, y, width, 3, 0xEEEEEE, 0xEEEEEE, 0xBBBBBB, 0xBBBBBB, localization["open_iris"])); y = y + 3
...
workspace.messageContactButton = workspace:addChild(GUI.framedButton(x, y, width, 3, 0xEEEEEE, 0xEEEEEE, 0xBBBBBB, 0xBBBBBB, localization["message"])); y = y + 4
workspace.messageContactButton.onTouch = function()
	local container = GUI.addBackgroundContainer(workspace, true, true, localization["message"])
	local input = container.layout:addChild(GUI.input(1, 1, 36, 3, 0xEEEEEE, 0x666666, 0x666666, 0xEEEEEE, 0x262626, nil, localization["type_message_here"]))
	input.onInputFinished = function()
		if input.text then
			container:remove()
...
workspace:addChild(GUI.label(x, y, width, 1, 0xEEEEEE, localization["contacts"])):setAlignment(GUI.ALIGNMENT_HORIZONTAL_CENTER, GUI.ALIGNMENT_VERTICAL_TOP); y = y + 2
workspace.contactsComboBox = workspace:addChild(GUI.comboBox(x, y, width, 3, 0x3C3C3C, 0xBBBBBB, 0x555555, 0x888888)); y = y + 4

workspace.connectContactButton = workspace:addChild(GUI.framedButton(x, y, width, 3, 0xEEEEEE, 0xEEEEEE, 0xBBBBBB, 0xBBBBBB, localization["connect"])); y = y + 3
...
workspace.addContactButton.onTouch = function()
	...
	local input2 = container.layout:addChild(GUI.input(1, 1, 36, 3, 0xEEEEEE, 0x666666, 0x666666, 0xEEEEEE, 0x262626, contacts.last, localization["address"]))
...
workspace:addChild(GUI.label(x, y, width, 1, 0xEEEEEE, localization["energy_to_dial"])):setAlignment(GUI.ALIGNMENT_HORIZONTAL_CENTER, GUI.ALIGNMENT_VERTICAL_TOP); y = y + 2
